...
void MainWindow::setupModel()
{
    ...
    //year edited directly in the view
...
void MainWindow::setupView()
{
    ...
    //validates year in view editing
...
void MainWindow::setupUI()
{
    ...
    //Attach validators
    m_authorEdit->setValidator(new AuthorValidator(this));
    m_titleEdit->setValidator(new TitleValidator(this));
    m_journalEdit->setValidator(new TitleValidator(this)); // same rules as title
    m_pagesEdit->setValidator(new PagesValidator(this));

    //Visual feedback: red border when input is invalid
    auto attachFeedback = [](QLineEdit* edit)
    {
        QObject::connect(edit, &QLineEdit::textChanged,
                         [edit](const QString& text)
                         {
                             QString t = text;
                             int pos = 0;
                             const QValidator* v = edit->validator();
                             bool ok = !v || v->validate(t, pos) != QValidator::Invalid;
                             edit->setStyleSheet(ok ? "" :
                                                     "QLineEdit { border: 1px solid red; }");
                         });
    };

    attachFeedback(m_authorEdit);
    attachFeedback(m_titleEdit);
    attachFeedback(m_journalEdit);
    attachFeedback(m_pagesEdit);
...
void MainWindow::colorRow(int row)
{
    ...
    if (age > 10)
        ...
        color = QColor(Qt::white);     // white — between 6 and 10 years
...
void MainWindow::addRecord()
{
    ...
    //Checks if a QLineEdit's content is Acceptable
    auto fieldOk = [](QLineEdit* edit) -> bool
    {
        if (!edit->validator()) return true;
        QString t = edit->text();
        int pos = 0;
        return edit->validator()->validate(t, pos) == QValidator::Acceptable;
    };

    // Validate author
    if (!fieldOk(m_authorEdit))
    {
        QMessageBox::warning(this, "Invalid Input",
                             "Author name is invalid.\n\n"
                             "Use letters, spaces and hyphens only.\n"
                             "Example: SZ Mbanjwa-Mhlana");
        m_authorEdit->setFocus();
        return;
    }

    // Validate title
    if (!fieldOk(m_titleEdit))
    {
        QMessageBox::warning(this, "Invalid Input",
                             "Article title is invalid.\n\n"
                             "Words must contain only letters OR only digits — not both.\n"
                             "Example: '3D' is not allowed.");
        m_titleEdit->setFocus();
        return;
    }

    // Validate journal name
    if (!fieldOk(m_journalEdit))
    {
        QMessageBox::warning(this, "Invalid Input",
                             "Journal name is invalid.\n\n"
                             "Words must contain only letters OR only digits — not both.");
    ...
    // Validate pages
    if (!fieldOk(m_pagesEdit))
    {
        QMessageBox::warning(this, "Invalid Input",
                             "Pages format is invalid.\n\n"
                             "Use the form '12-15' or '121 - 155'.\n"
                             "The end page must be greater than or equal to the start page.");
        m_pagesEdit->setFocus();
        return;
    }

    // All valid — build and append the row
    ...
    // Store numeric year as UserRole so sorting works correctly
...
void MainWindow::removeRecord()
{
    ...
    // Map proxy index back to source model before removing
...
void MainWindow::applyFilter()
{
    ...
    // Convert * and ? pattern to a full regular expression
    ...
    // Pages is excluded from the combo so indices match directly
...
//Handle in-view data edits
void MainWindow::onDataChanged(const QModelIndex& topLeft,
                               const QModelIndex& bottomRight,
                               const QList<int>&)
{
    // Recolour every row that was changed
